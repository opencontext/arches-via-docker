...
copy_her_template_and_media() {
	echo "Copy HER package templates and media files..."
...
start_celery_supervisor() {
	...
	if [ -f "/tmp/supervisor.sock" ]; then
		...
		cd ${APP_FOLDER}
		wait-for-it arches_redis:6379 -t 120 && supervisord -c arches_local_her_v7-supervisor.conf
	fi
}
...
run_setup_webpack() {
	...
	if [[ ! -d ${STATIC_JS} ]] || [[ ! "$(ls ${STATIC_JS})" ]]; then
		...
		if [[ ${BUILD_PRODUCTION} == 'True' ]]; then
			...
			exec sh -c "yarn install && yarn add cross-env && yarn add jquery-validation && yarn build_development && python3 $APP_FOLDER/manage.py collectstatic --noinput"
		fi
