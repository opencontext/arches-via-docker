...
start_celery_beat() {
	...
	if [ -f "/var/log/celery/beat.log" ]; then
		...
		cd ${APP_FOLDER}
		exec sh -c "wait-for-it arches_rabbitmq:5672 -t 120 && celery -A arches_proj.celery beat --schedule=/tmp/celerybeat-schedule --loglevel=INFO --pidfile=/tmp/celerybeat.pid"
	fi
}
